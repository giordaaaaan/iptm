...
const filters = ref({
  ...
  filename: '',
  processor: '',
  status: '' // '' means All, or "New", "Assigned", or "Cancelled"
});
...
const filteredInvoices = computed(() => {
  return invoices.value.filter(invoice => {
    ...
    const statusValid = !filters.value.status || invoice.status === filters.value.status;
    
    // Check Created date range (compare first 10 characters)
    ...
    return textValid && statusValid && createdValid && processedValid;
  });
});

// Navigation action for processor "Show"
const showInvoice = (invoice) => {
  router.push(`/invoice/${invoice.iid}/show/processor`);
};
</script>
