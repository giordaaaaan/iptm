<template>
  ...
  // Computed maximum allowable "Created to" date (1-month limit)
  ...
  // Filter invoices based on text filters and created date range
  const filteredInvoices = computed(() => {
    return invoices.value.filter(invoice => {
      ...
      // Date filter: compare only the date portion (first 10 characters) of the actionDate
      let dateValid = true;
      if (createdFrom.value) {
        const invDate = invoice.actionDate.substring(0, 10);
        dateValid = dateValid && invDate >= createdFrom.value;
      }
      if (createdTo.value) {
        const invDate = invoice.actionDate.substring(0, 10);
        dateValid = dateValid && invDate <= createdTo.value;
      }
